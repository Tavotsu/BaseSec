    ...
    visit(ctx.sourceFile, (node) => {
      if (ts.isCallExpression(node)) {
        ...
        if (text.includes('cors') && text.includes('origin') && text.includes('*')) {
          ...
          const hasCredentialsTrue = text.includes('credentials:') && (text.includes('credentials: true') || text.includes('credentials:true') || text.includes("credentials: 'true'") || text.includes("credentials:'true'"));
          findings.push({
            ruleId: 'AUTH-003',
            ruleName: 'Permissive CORS Configuration',
            category: 'auth',
            severity: hasCredentialsTrue ? 'high' : 'medium',
            filePath: ctx.filePath,
            line,
            column,
            endLine: line,
            endColumn: column + text.length,
            message: hasCredentialsTrue
            ...
            confidence: hasCredentialsTrue ? 'high' : 'medium',
          });
        }
      }
      return 'continue';
