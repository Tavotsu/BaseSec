...
const SINK_DEFINITIONS: SinkDefinition[] = [
  ...
  { pattern: '$queryRaw', category: 'sql-injection' },
  { pattern: '$executeRaw', category: 'sql-injection' },
  { pattern: '$queryRawUnsafe', category: 'sql-injection' },
  { pattern: '$executeRawUnsafe', category: 'sql-injection' },
...
const SINK_METHOD_RECEIVERS: Record<string, string[]> = {
  ...
  $queryRaw: ['prisma', 'PrismaClient', 'db'],
  $executeRaw: ['prisma', 'PrismaClient', 'db'],
  $queryRawUnsafe: ['prisma', 'PrismaClient', 'db'],
  $executeRawUnsafe: ['prisma', 'PrismaClient', 'db'],
  find: ['Model', 'model', 'collection'],
  $where: ['Model', 'model', 'collection'],
  send: ['res', 'reply', 'ctx', 'ctx.body'],
  json: ['res'],
  redirect: ['res', 'reply', 'ctx', 'ctx.response'],
  set: ['res'],
  setHeader: ['res'],
